...
| `CLIENT_CHECK_INTERVAL`   | `15`                       | `INTEGER` (minutes)      | How often to try a connection to clients with a valid IP and port.                                                                          |
...
- The API web server runs on port 8081 inside the container to expose the RESTful API which clients use for various
  purposes.
...
Browse to `http://hostname:8081/admin` to set up the admin password.
